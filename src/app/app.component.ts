...
@Component({
  selector: 'app-root',
  template: `
    <mat-toolbar color="primary" fxLayoutGap="8px" *ngIf="{status: authService.authStatus$ |async, user: authService.currentUser$ | async} as auth">
      <button mat-icon-button><mat-icon  *ngIf="auth?.status?.isAuthenticated">menu</mat-icon><mat-icon svgIcon="lemon"></mat-icon></button>
